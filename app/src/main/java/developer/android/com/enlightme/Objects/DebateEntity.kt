...
@Serializable
class DebateEntity: BaseObservable() {
    @Bindable
    var path_to_root = LiveMutableList<LiveMutableList<Int>>() // gives the path from the root to
    // reach this argument
        set(value) {
            field = value
            notifyPropertyChanged(BR.path_to_root)
        }
    @Bindable
    var title = ""
        set(value) {
    ...
    var side_1 = "Side 1"
        set(value) {
            field = value
            notifyPropertyChanged(BR.side_1)
        }
    @Bindable
    var side_2 = "Side 2"
        set(value) {
            field = value
            notifyPropertyChanged(BR.side_2)
        }
    @Bindable
    var side_1_entity = LiveMutableList<DebateEntity>()
        set(value) {
            Log.i("DebateEntity", "side_1_entity set function")
            field = value
            notifyPropertyChanged(BR.side_1_entity)
            Log.i("DebateEntity", "side 1 changed")
        }
    @Bindable
    var side_2_entity  = LiveMutableList<DebateEntity>()
        set(value) {
            field = value
            notifyPropertyChanged(BR.side_2_entity)
        }
    ...
    var updateWaitingList = mutableListOf<UpdatePayload>()
    ...
    fun setSubDebateEntity(trace: LiveMutableList<LiveMutableList<Int>>,
                           newDebateEntity: DebateEntity){
        val currentPos = trace.removeAt(0)
        if(trace.size > 0){
            if(currentPos[0] == 1){
                this.side_1_entity[currentPos[1]].setSubDebateEntity(trace, newDebateEntity)
            }else{
                this.side_2_entity[currentPos[1]].setSubDebateEntity(trace, newDebateEntity)
            }
        }else{
            if(currentPos[0] == 1){
                this.side_1_entity[currentPos[1]] = newDebateEntity
            }else{
                this.side_2_entity[currentPos[1]] = newDebateEntity
            }
        }
    }
}